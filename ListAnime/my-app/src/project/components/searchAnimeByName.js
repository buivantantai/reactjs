...
export default function SearchAnimeByName() {
  let thisPage;
  let limit = 25;
  let list = document.querySelectorAll(".tab-content-list .card");
  let count;
  ...
  const changePage = (i) => {
    navigate(`/search/${params.name}/page/${i}`);
    refresh();
  };
  const returnPage = () => {
    navigate(`/search/${params.name}`);
    refresh();
  };
  ...
  const getListAnimeByName = async (name) => {
    const result = await axios.get(`https://api.jikan.moe/v4/anime?q=${name}`);
    setAnimeLocal(result.data);
  };

  useEffect(() => {
    getListAnimeByName(params.name);
  }, []);
      ...
      >
        ...
        >
          <div className="box-item">
            <div>
              <h1>
                <span>Your Search: {params.name}</span>
              </h1>
