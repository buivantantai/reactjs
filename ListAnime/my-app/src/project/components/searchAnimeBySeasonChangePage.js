...
export default function SearchAnimeBySeasonChangePage() {
  let thisPage;
  let count;
  ...
  const changePage = (i) => {
    navigate(`/year/${params.year}/season/${params.season}/page/${i}`);
    refresh();
  };
  const returnPage = () => {
    navigate(`/year/${params.year}/season/${params.season}`);
    refresh();
  };
  ...
  const getListAnimeBySeason = async (year, season) => {
    const result = await axios.get(
      `https://api.jikan.moe/v4/seasons/${year}/${season}?page/${params.page}`
    );
    setAnimeLocal(result.data);
  };
  useEffect(() => {
    getListAnimeBySeason(params.year, params.season);
  }, []);
  useEffect(() => {
